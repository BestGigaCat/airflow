...
def preprocess_glue():
    ...
    jobrun_resp = glue_client.start_job_run(
    ...
    while True:
        status = glue_client.get_job_run(JobName=response['Name'], RunId=jobrun_resp['JobRunId'])
...
sess = hook.get_session(
    region_name=config.REGION_NAME)  # how is this session different from the SageMaker session - necessary?
...
sagemaker_validation_data = s3_input(config.SAGEMAKER_VALIDATION_DATA_S3_SOURCE,
